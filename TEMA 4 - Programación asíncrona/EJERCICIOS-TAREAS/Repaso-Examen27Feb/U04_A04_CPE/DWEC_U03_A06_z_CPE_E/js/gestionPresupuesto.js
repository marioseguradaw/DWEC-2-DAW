...
listaGastos.addEventListener('click', async (e) => {
    // Si el elemento clickeado tiene la clase 'btn-borrar'
...
async function cargarGastosAPI() {
    try {
        ...
        console.error(error);
        alert("Error cargando los datos. Revisa que el servidor (npm start) esté encendido.");
    }
}
...
function renderizarLista(gastos) {
    ...
    if (!gastos || gastos.length === 0) {
        listaGastos.innerHTML = '<p style="text-align:center; color:888;">No hay gastos registrados.</p>';
        return;
    }
